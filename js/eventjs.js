$(function() {
    ...
    $("#songName").keydown(function(e) {
        switch (e.which) {

            case 40: // down
            if ($('#songSuggest').text() != '') {
                $('#songName').val(trackName);
                $('#artistName').val(trackArtist);
            }
            break;
            case 9: //tab
            if ($('#songSuggest').text() != '') {
    ...
    $("#songName").keyup(function(e) {
        ...
        switch (e.which) {
            ...
            trackSearch = $("#songName").val();
                // empty search
                if (trackSearch == '') {
                    albArtHTML = '<img src="noArt.png">';
                    $("#modalArt").html(albArtHTML);
                    $('#artistName').val('');
                } else
                searchMusic(trackSearch);
            }
        })

//search music and set variables for track, artist, art
var searchMusic = function(query) {
    $.ajax({
        ...
        success: function(response) {
            trackObj = response.tracks.items[0];
            if (trackObj != undefined) {
                trackName = trackObj.name;
                trackArtist = trackObj.artists[0].name;
                trackAlbArt = trackObj.album.images[0].url;

                    // fill in track name
                    $('#songSuggest').html(trackName + ' - ' + trackArtist);

                    //  fill in track artist
                    // $('#artistName').val(trackArtist);

                    // fill in album art
                    albArtHTML = '<img src="' + trackAlbArt + '">';
                    $('#modalArt').html(albArtHTML);
                }
    ...
    var parsePost = newPost.save({
        ...
        success: function(object) {
            console.log('we made it');
                ...
                <div class="albumArt">' + albArtHTML + '</div>\
                <div class="allText">\
                ...
                //reset form inputs, etc
                $('#songName').val('');
                $('#artistName').val('');
                $('#songSuggest').text('');
                $("#modalArt").html('<img src="noArt.png">');
...
function fauxFor(counter) {
    ...
    albumArt = trueResults[counter].attributes.albumArt;
    putinType = '';
    ...
    querydidI.find({
        success: function(object) {
            ...
            <div class="albumArt">' + albumArt + '</div>\
            <div class="allText">\
